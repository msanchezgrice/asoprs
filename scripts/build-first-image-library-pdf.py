...
def build_pdf(manifest_path: Path, output_path: Path) -> None:
    ...
    section_title = ascii_text(manifest["sectionTitle"])
    ...
    canvas.setTitle(f"ASOPRS Image Library - {section_title}")
    canvas.setAuthor("ASOPRS Study Portal")
    canvas.setSubject(f"{section_title} image library")
    ...
    for page_index, entry in enumerate(entries, start=1):
        ...
        canvas.drawString(50, 496, ascii_text(entry["documentTitle"]))
        canvas.setFillColor(CORAL)
        canvas.setFont("Helvetica-Bold", 10)
        label = ascii_text(
            f'{entry["figureLabel"]} - Source page {entry["pageNumber"]}'
        )
        canvas.drawRightString(910, 498, label)

        canvas.setFillColor(IVORY)
        canvas.setStrokeColor(IVORY_DARK)
        canvas.roundRect(50, 55, 560, 410, 12, fill=1, stroke=1)
        image_path = project_root / "public" / entry["imagePath"].lstrip("/")
        draw_contained_image(canvas, image_path, 64, 69, 532, 382)
        ...
        canvas.drawRightString(
            910,
            32,
            f"{section_title} - {page_index} of {len(entries)}",
        )
        canvas.showPage()
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: build-first-image-library-pdf.py <manifest.json> <output.pdf>"
        )
